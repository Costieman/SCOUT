...
<div class="card"><h2>3. Exit candidates to compare</h2><div class="section-note">Hold-to-horizon is always the control. Add any fixed, trailing, ATR, or trailing-ATR stops you want to compare against it.</div><div id="stop-rows"></div><div class="toolbar"><button id="add-stop" type="button">+ Add exit candidate</button><button id="clear-stops" type="button">Clear all stops</button><span class="subtle">Hold-to-horizon remains even when no stop rows are present.</span></div></div>
