"""Core application service for one-variable parameter sweeps of entry indicators."""
        ...
        *,
        ...
        if base_request.entry_family.value != "feature_expression":
            raise StrategyBuilderError(
                "entry-parameter sweeps require the feature-expression entry family"
            )
        if not isinstance(self.source, WindowedStrategyBuilderSource):
            raise StrategyBuilderError(
                "entry-parameter sweeps require the window-aware canonical source"
            )
        if not values:
        ...
        if base_spec is None:
            raise ValueError(
                "entry sweep target must be a parameterized indicator in the current entry definition"
            )
...
def _replace_parameter(
    ...
    if parameter is EntrySweepParameter.PERIOD:
        return replace(spec, period=integer)
    if parameter is EntrySweepParameter.FAST_PERIOD:
        return replace(spec, fast_period=integer)
    if parameter is EntrySweepParameter.SLOW_PERIOD:
        return replace(spec, slow_period=integer)
    if parameter is EntrySweepParameter.SIGNAL_PERIOD:
        return replace(spec, signal_period=integer)
