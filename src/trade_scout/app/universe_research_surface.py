...
a {{ color:var(--accent); text-decoration:none; }} .wrap {{ width:min(1600px,96vw); margin:0 auto; padding:28px 0 70px; }} header {{ display:flex; justify-content:space-between; gap:20px; align-items:flex-start; margin-bottom:18px; }} h1 {{ margin:0; font-size:28px; }} h2 {{ margin:0 0 10px; font-size:18px; }} .subtle {{ color:var(--muted); }}
...
@media(max-width:1200px) {{ form {{ grid-template-columns:1fr 1fr; }} .s3,.s4,.s6,.s8 {{ grid-column:1/-1; }} }}
</style>
</head>
<body><div class="wrap">
<header><div><a href="/">← Research console</a><h1>Universe Research Analyzer</h1><div class="subtle">Pattern timeframe, market-wide opportunity frequency and daily holding outcomes in one experiment.</div></div><span class="pill">EXPLORATORY RESEARCH v0.2</span></header>
<div class="banner"><strong>Research, not a signal generator.</strong> Pattern-bar timeframe and holding horizon are separate. A weekly setup can therefore be evaluated at a 2, 5, 20 or 60 daily-session holding horizon without redefining the setup.</div>
<div class="banner scope"><strong>Current universe boundary:</strong> this uses the fully reviewed canonical equity scope available in the private workspace. It does not pretend this is historical point-in-time S&amp;P 500 membership.</div>
<div class="card">
...
def _empty_state(error: str | None) -> str:
    if error:
        return ""
    return """<div class="card" style="margin-top:14px"><h2>Define the baseline experiment</h2><div class="subtle">Choose daily, 2-session, 3-session or weekly pattern bars. The analyzer scans the full available universe, counts setups by month, measures daily-session outcomes, shows cross-stock breadth and builds a nearby duration x consolidation-range surface.</div></div>"""
...
<div class="grid" style="margin-top:14px">
  ...
  <div class="card s3"><div class="metric-label">{report.selected_horizon}-daily-session mean</div><div class="metric {_value_class(selected.mean_return)}">{_pct(selected.mean_return)}</div><div class="subtle">n={selected.sample_size} complete outcomes</div></div>
  ...
  <div class="card s3"><div class="metric-label">Event concentration</div><div class="metric">{_prob(report.top_five_event_share)}</div><div class="subtle">share from the five most active stocks</div></div>
