...
class ResolvedIdentityBatch:
    """A deduplicated set of ready identity evidence from independent resolver layers."""
...
def load_resolved_identity_batch(
    *,
    ...
    """Load the four second-pass READY cases plus SEC historical-index READY cases."""
        ...
        for key in _symbol_aliases(source):
            ...
            if previous is not None and previous is not row:
                raise ResolvedIdentityBatchError(
                    f"ambiguous deferred context for symbol alias {key}"
                )
            context[key] = row

    assembled: list[AutoIdentityEvidence] = []
    for row in deferred_rows:
        if _text(row.get("status"), "deferred status").upper() != "READY":
            raise ResolvedIdentityBatchError("deferred ready file contains a non-READY row")
        assembled.append(_evidence_from_deferred_resolution(row))
    ...
    for row in historical_rows:
        ...
        source_row = next(iter(matches.values()))
        assembled.append(_evidence_from_historical(row, source_row))
        historical_count += 1
...
def _evidence_from_deferred_resolution(row: dict[str, object]) -> AutoIdentityEvidence:
    cik = _required_int(row.get("cik"), "deferred cik")
    company_name = _text(row.get("company_name"), "deferred company_name")
    exchange = _text(row.get("exchange"), "deferred exchange")
    evidence_url = _text(row.get("evidence_url"), "deferred evidence_url")
    return AutoIdentityEvidence(
        source_symbol=_text(row.get("source_symbol"), "deferred source_symbol").upper(),
        ...
        evidence_kind=_text(row.get("resolution_kind"), "resolution_kind"),
        ready=True,
        reason=_text(row.get("reason"), "deferred reason"),
    )
...
def _evidence_from_historical(
    ...
    if historical_cik != source_cik:
        raise ResolvedIdentityBatchError(
            "historical READY CIK differs from the deferred-resolution CIK"
        )
