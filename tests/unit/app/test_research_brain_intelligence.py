...
def _view(*, failed_count: int = 0, drift_warning_count: int = 0):
    ...
    manifest = SimpleNamespace(
        ...
        experiment_id="exp-1",
        definition=SimpleNamespace(resolved_configuration={"holding_horizon": 20}),
    )
    experiment = SimpleNamespace(manifest=manifest, result=None, stage_outputs={})
    item = SimpleNamespace(experiment=experiment, integrity_error=None)
    return SimpleNamespace(snapshot=snapshot, experiments=(item,))


def test_synthesis_recommends_next_missing_stage() -> None:
