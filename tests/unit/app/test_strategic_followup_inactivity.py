...
def _row(target: float, expectancy: float, hit: float) -> ExitPolicySummary:
    return ExitPolicySummary(
        policy_id=str(target),
        policy_version="v1",
        family=ExitFamily.FIXED_PERCENT_STOP,
        resolved_parameters={"distance_pct": 0.05},
        target_family=TargetFamily.FIXED_PERCENT,
        target_parameters={"gain_pct": target},
        sample_size=2892,
        stop_out_count=1420,
        stop_out_rate=0.491,
        target_hit_count=int(hit * 2892),
        target_hit_rate=hit,
        ...
        profit_factor=1.4,
        tail_loss_p05=-0.055,
        average_holding_period_sessions=13.7,
        median_holding_period_sessions=20.0,
        median_mae_before_exit=-0.04,
        median_mfe_full_horizon=0.08,
        median_max_drawdown_before_exit=-0.05,
